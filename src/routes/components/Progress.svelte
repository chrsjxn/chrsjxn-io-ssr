<script>
  const random = (max) => Math.round(Math.random() * max)
  const points = new Array(1000).fill().map(() => [
    {
      x: random(6000),
      y: random(3810),
    },
    {
      x: random(6000),
      y: random(3810),
    },
    {
      x: random(6000),
      y: random(3810),
    },
  ])

</script>

<style>
  svg {
    max-width: 100vw;
    max-height: 100vh;
  }

</style>

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 6000 3810">
  <title>
    Progress pride flag, rendered with a scratchy mask to appear worn.
  </title>
  <defs>
    <mask id="Mask">
      <g stroke="white" stroke-width="8" stroke-linecap="round" fill="none">
        {#each points as point}
          <polygon
            points={`${point[0].x} ${point[0].y}, ${point[1].x} ${point[1].y}, ${point[2].x} ${point[2].y}`} />
        {/each}
      </g>
    </mask>
  </defs>
  <g mask="url(#Mask)">
    <rect fill="#750787" width="6000" height="3810" />
    <rect fill="#004dff" width="6000" height="3175" />
    <rect fill="#008026" width="6000" height="2540" />
    <rect fill="#ffed00" width="6000" height="1905" />
    <rect fill="#ff8c00" width="6000" height="1270" />
    <rect fill="#e40303" width="6000" height="635" />

    <polygon points="-1,960 950,1905 -1,2850" fill="#ffffff" />
    <polygon
      points="-1,960 -1,480 1420,1905 -1,3330 -1,2850 950,1905"
      fill="#ffafc8" />
    <polygon
      points="-1,480 -1,0 1912,1905 -1,3810 -1,3330 1420,1905 -1,480"
      fill="#74d7ee" />
    <polygon
      points="-1,0 1912,1905 -1,3810 480,3810 2384,1905 480,0"
      fill="#613915" />
    <polygon
      points="480,0 960,0 2864,1905 960,3810 480,3810 2384 1905 480,0"
      fill="#000000" />
  </g>
</svg>
